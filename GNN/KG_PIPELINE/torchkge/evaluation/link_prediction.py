...
class LinkPredictionEvaluator(object):
    ...
    * Antoine Bordes, Nicolas Usunier, Alberto Garcia-Duran, Jason Weston,
    ...
    def evaluate(self, b_size, verbose=True, k=10):
        """
        ...
        summary ={}
        summary['Normal'] = {f'Hit@{k}': round(self.hit_at_k(k=k)[0], 4), 
                             'Mean Rank': int(self.mean_rank()[0]), 
                             'MRR': round(self.mrr()[0], 4)}
        
        summary['Filtered'] = {f'Hit@{k}': round(self.hit_at_k(k=k)[1], 4), 
                               'Mean Rank': int(self.mean_rank()[1]), 
                               'MRR': round(self.mrr()[1], 4)}
        return summary
